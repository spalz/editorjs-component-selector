...
export default class ComponentSelectorTool {
  ...
  makeSelector() {
    ...
    for (let component of this.config.components) {
      const option = this.makeElement("option", [this.makeClass("option")], {
  ...
  renderPreview() {
    if (!this.nodes.preview) {
      ...
      this.nodes.preview = this.makeElement("img", [this.makeClass("preveiw")]);
